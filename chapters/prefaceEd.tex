...
\title{% \textit{Who nicks the nickname?} --
  An interdisciplinary, cross-lingual perspective on the role of constituents in multiword expressions}

\author{%
  Sabine Schulte im Walde\affiliation{Institut für Maschinelle Sprachverarbeitung, Universität Stuttgart}\and
  Eva Smolka\affiliation{Fachbereich Sprachwissenschaft, Universität Konstanz}
}
...
\abstract{Multiword expressions (MWEs), such as noun compounds (such
...
relatively semantically transparent as in \textit{Familienname}
(`family name') to semantically opaque, as in \textit{Spitzname}
(`nickname', lit. `pointy name') or \textit{Zwickmühle} (`dilemma',
lit. `pinch mill'), which contain a modifier (i.e. the left
constituent) or a head (i.e. the right constituent) that renders the
...
In the following, the six contributions in this volume discuss MWEs
that are composed of different types of constituents, including the
combination of particle+stem in complex verbs (e.g.,
\textit{aufstehen}, `stand up’), the combination of stem+ stem and
stem+preposition+stem in noun compounds (e.g., \textit{nickname,
  government assessment, juego de niños}), the combination of
...
Seminal psycholinguistic studies have applied the manipulation of
...
Findings in German, however, indicate that lexical processing in
German occurs via the stem and irrespective of semantic transparency
...
into models of
compositionality. \cite{Kuehner/SchulteImWalde:10,Bott/SchulteImWalde:17,Koeper/SchulteImWalde:17a}
...
\cite{AedmaaEtAl:18} applied classifiers to identify non-literal
...
\textit{Discourse Representation Theory} \citep{Kamp/Reyle:93} for
German particle verbs with the particles \textit{auf}
\citep{Lechler/Rossdeutscher:09}, \textit{ab} \citep{Kliche:11},
\textit{an} \citep{Springorum:11} and \textit{nach}
\citep{Haselbach:11}, this gap of knowledge has recently been
...
Also compounds lie on a continuum between relatively transparent and
...
compounds can be subdivided into two subfields: (i) approaches that
aim to predict the \textit{meaning} of a compound by composite
functions, relying on the vectors of the constituents (e.g.,
\citealt{Mitchell/Lapata:10,CoeckeEtAl:11,BaroniEtAl:14b,Hermann:14});
and (ii) approaches that aim to predict the \textit{degree of
  compositionality} of a compound, typically by comparing the compound
vectors with the constituent vectors (e.g.,
\citealt{ReddyEtAl:11a,Salehi/Cook:13,SchulteImWaldeEtAl:13,SalehiEtAl:14,SalehiEtAl:14b,SalehiEtAl:15,SchulteImWaldeEtAl:16b,Koeper/SchulteImWalde:17b}). As
for complex verbs, the computational models typically rely to a large
...
MWEs and their constituents, such as ambiguity, frequency,
...
hand each, an idiom is composed of single constituents that are
...
the meaning of the constituents adds to the figurative meaning. They
