...
\lsptoprule  Frequency & AL & ANCE & (AT)ION & ING & MENT \\ \midrule
...
Second, we \textit{do} find \textsc{subj}-DCs headed by \textit{ing}-nouns (see~\ref{1}b). In general, the information on the suffix is independent of ASN-hood, since all suffixes allow both ASN and RN readings, but we aimed to check whether some suffixes may be more informative than others. 
...
in Table~\ref{tab:prediction:all} below indicate that high compoundhood correlates with an \textsc{obj} interpretation of the non-head, which supports the relevance of compositionality in the formation of DCs.
