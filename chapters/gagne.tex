...
\abstract{We examined the underlying psycholinguistic and cognitive
  factors that give rise to the production of multiword expressions by
  examining what might prompt people to include modifying information
  when referring to recently encountered information. For example, if
  a story describes a woman buying a dog with blue fur, will people
  include the color of the dog when referring to the animal? If they
  do include that information, what syntactic form will they use? In
  the experiment, participants read short stories that contained a
  concept that was presented as either a modifier-noun phrase (e.g.,
  the blue dog) or full phrase (the dog that was blue).  We varied
  whether the property being highlighted was normal (e.g., brown) or
  distinctive (e.g., blue) for the head noun concept. The concept was
  the target antecedent. We found that participants are more likely to
  include distinctive properties than normal properties when referring
  to the concept. In addition, although the selection of a syntactic
  form was partially influenced by the form of the information in the
  story, there was a strong overall bias toward using a modifier-noun
  phrase structure.}
...
\begin{table}
  \caption{Number of responses and row percentages (in parentheses)
    ...
    head noun only, or other. Each row sums to 364.\todo[inline]{Can we use an abbreviation for ``modifier-noun phrase'' (e.g. mod-n phrase) in the table heading?}}
